...
import { type RetencionesData } from '../retenciones-data.js';
...
export class GenericRetencionesTranslator implements DocumentTranslatorInterface<RetencionesData> {
  ...
  protected generatePeriodoContent(periodo: CNodeInterface, claveReturnValue: string): Content {
    return {
      style: 'tableContent',
      table: {
        widths: ['*', '*', '*', '*'],
  ...
  protected generateImpuestosRetenidos(impuestosRetenidos: CNodeInterface[]): TableCell[][] {
    ...
    for (const impuestoRetenido of impuestosRetenidos) {
      rowsRetenciones.push([
  ...
  protected generateContent(data: RetencionesData): Content {
    ...
    content.push(
      this.generateTopContent(retenciones, data.logo()),
      '\n',
      this.generateEmisorContent(emisor),
      '\n',
      this.generateReceptorContent(receptor),
      '\n',
      '\n',
      this.generatePeriodoContent(periodo, retenciones.get('CveRetenc')),
      '\n',
      this.generateTotales(totales),
      '\n',
    );
