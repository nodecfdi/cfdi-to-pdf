...
const genericRelatedInfoContent = (
  ...
  const totalImpuestosTrasladados = data
    .comprobante()
    .searchAttribute('cfdi:Impuestos', 'TotalImpuestosTrasladados');
  const totalImpuestosRetenidos = data
    .comprobante()
    .searchAttribute('cfdi:Impuestos', 'TotalImpuestosRetenidos');
      ...
      table: {
        ...
        body: [
          [
            ...
            'Subtotal:',
            { text: formatCurrency(data.comprobante().getAttribute('SubTotal')) },
          ],
          [
            {
              text: getValueOfCatalog(
                'cfdi40MetodosPago',
                data.comprobante().getAttribute('MetodoPago'),
                _catalogs,
              ),
              alignment: 'left',
            },
            'Descuento:',
            formatCurrency(data.comprobante().getAttribute('Descuento')),
          ],
          [
          ...
          [
            {
              text: getValueOfCatalog(
                'cfdi40FormasPago',
                data.comprobante().getAttribute('FormaPago'),
                _catalogs,
              ),
              alignment: 'left',
            },
